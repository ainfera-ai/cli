...
def deploy(
    ...
    sandbox = compute.get("sandbox", "sandbox")
    ...
    try:
        ...
        if agent_id:
            ...
            if trust_data is None or not (
                ...
                # Set baseline if no score yet (tolerate endpoint absence)
                try:
...
def _format_sandbox_detail(sandbox: str, memory: str, cpu: int) -> str:
    ...
    label = sandbox.capitalize() if isinstance(sandbox, str) else "Sandbox"
...
def _run_demo(*, json_output: bool) -> None:
    ...
    console.print(
        Panel(
            "\n"
            "  Agent:     [bold]my-agent[/]\n"
            "  Framework: [ainfera.muted]auto-detected[/]\n"
    ...
    steps = [
        ("Provisioning sandbox...", "512MB, 1 CPU"),
