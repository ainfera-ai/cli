...
# Safety is evaluated on the server via NVIDIA NeMo Guardrails; the other
# dimensions are scored by NVIDIA NIM inference.
DIMENSION_SOURCE = {
    "safety": "via NeMo Guardrails",
}
...
def _print_trust_radar(score: int, grade: str, dimensions: dict[str, float]) -> None:
    ...
    for dim in DIMENSIONS:
        ...
        source = DIMENSION_SOURCE.get(dim, "")
        source_tag = f"  [ainfera.muted]{source}[/]" if source else ""
        label = dim.capitalize().ljust(12)
        console.print(f"  {label} {value:.2f}  {bar}{source_tag}")
    console.print()
    console.print(
...
def _build_pr_comment(
    ...
    for dim in DIMENSIONS:
        if dim not in dimensions:
            continue
        src = "NVIDIA NeMo Guardrails" if dim == "safety" else "NVIDIA NIM"
        lines.append(f"| {dim.capitalize()} | {dimensions[dim]:.2f} | {src} |")
    lines.append("")
    lines.append("_Powered by NVIDIA NIM and NeMo Guardrails._")
    return "\n".join(lines)
